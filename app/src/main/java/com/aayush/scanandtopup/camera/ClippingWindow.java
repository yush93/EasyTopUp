    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs((VelocityTrackerCompat.getXVelocity(mVelocityTracker, pointerId)))>30|
                        Math.abs((VelocityTrackerCompat.getYVelocity(mVelocityTracker, pointerId)))>30)
                {
                    leftCroppable=rightCroppable=topCroppable=bottomCroppable=false;
                    draggable = false;
                }

                if (left <= leftBoundary & moveX<0) {leftCroppable = false;}

                if (right >= rightBoundary & moveX>0) {rightCroppable = false;}
                if (top <= topBoundary & moveY<0) {topCroppable = false;}
                if (bottom >= bottomBoundary & moveY>0) {bottomCroppable = false;}
                ...
                if (draggable) {
                    ...
                    if (topCroppable) {top =  Math.max(top + moveY, topBoundary);}
                    if (bottomCroppable) {bottom = Math.min(bottom + moveY, bottomBoundary);}
                    if (leftCroppable) {left = Math.max(left + moveX, leftBoundary);}
                    if (rightCroppable) {right = Math.min(right + moveX, rightBoundary);}
                }
